...
IMAGE_PATH = "/home/el-ismaiyly/Images/PP.jpg"
...
def get_instances_from_vision(vision_result):
    ...
    # ❌ rien trouvé → debug clair
    raise ValueError(
        "Format agent vision inconnu. "
        f"Clés reçues: {list(vision_result.keys())}"
    )
...
#2fev
def call_vision_on_crop(agent_card, full_pil_img: Image.Image, crop_box,
                        target="person", multi=True, max_instances=5, conf=0.12):
    """
    crop_box = (x1,y1,x2,y2) en coordonnées image FULL
    retourne instances remappées vers FULL + meta _mask_space
    """
    x1, y1, x2, y2 = map(int, crop_box)
    crop = full_pil_img.crop((x1, y1, x2, y2))
    crop_b64 = pil_to_png_b64(crop)

    vr = call_vision_agent(agent_card, crop_b64, target=target,
                           multi=multi, max_instances=max_instances, conf=conf)
    insts = get_instances_from_vision(vr)
...
def build_smart_inpaint_actions(instance):
    ...
    m_roi, ox, oy = sam_mask_from_instance_bbox(
        image_path=IMAGE_PATH,
        bbox=bbox,
        pad=15
    )

    # (optionnel) postprocess très léger
    m_roi = _postprocess_mask(m_roi, close_px=3)   # pas de dilate large

    # debug
    _save_debug_masks(m_roi, "/tmp/final_mask_rgba.png", "/tmp/final_mask_L.png")

    mask_png_b64 = _mask_to_png_b64_rgba(m_roi, feather_radius=2.0, dilate_px=0)

    return [
        {
            "action": "select_mask_png",
            "target": "image",
            "params": {"png_b64": mask_png_b64, "offset_x": int(ox), "offset_y": int(oy)},
            "notes": "Sélection personne (SAM local bbox+pad, anti-débordement)"
        },
        {"action": "smart_inpaint", "target": "image", "params": {}, "notes": "Inpainting via LaMa"},
...
def sam_mask_from_instance_bbox(image_path: str, bbox: dict, pad: int = 25) -> tuple[np.ndarray, int, int]:
    """
    Retourne (mask_roi01, offset_x, offset_y)
    - On fait un CROP bbox+pad
    - On lance SAM sur le crop
    - On fournit points + et -
    - On choisit le meilleur mask (celui qui colle le plus à la bbox et évite le fond)
    """
    with Image.open(image_path).convert("RGB") as im:
        full = np.array(im, dtype=np.uint8)
    ...
    # bbox dans l'espace CROP
    bx1 = x - x1
    by1 = y - y1
    bx2 = bx1 + w
    by2 = by1 + h

    # --- Points POSITIFS (dans la bbox, mais pas au centre exact)
    # on place 3 points "sûrs" : haut, milieu, bas (légèrement décalés)
    px = int(bx1 + 0.65 * w)
    p1 = [px, int(by1 + 0.20 * h)]
    p2 = [px, int(by1 + 0.55 * h)]
    p3 = [px, int(by1 + 0.85 * h)]

    # --- Points NEGATIFS (fond) : à gauche et en haut du crop (souvent ciel)
    n1 = [max(2, int(bx1 - 10)), int(by1 + 0.50 * h)]
    n2 = [int(min(cw - 3, bx2 + 10)), int(by1 + 0.10 * h)]  # souvent ciel
    n3 = [int(min(cw - 3, bx2 + 10)), int(by1 + 0.90 * h)]  # fond/sol

    point_coords = np.array([p1, p2, p3, n1, n2, n3], dtype=np.float32)
    point_labels = np.array([1, 1, 1, 0, 0, 0], dtype=np.int32)

    # SAM sur CROP
    _sam_predictor.set_image(crop)
    ...
    # - max pixels dans bbox
    # - min pixels hors bbox (dans le crop)
    best = None
    best_score = -1e9
    ...
    for i in range(masks.shape[0]):
        ...
        inside = m[by1:by2, bx1:bx2].sum()
        total  = m.sum()
        outside = max(0, total - inside)

        # score: on veut beaucoup dedans, peu dehors
        s = float(inside) - 1.5 * float(outside)

        # bonus si le masque ne touche pas trop les bords du crop (souvent signe "fond")
        border = (m[0,:].sum() + m[-1,:].sum() + m[:,0].sum() + m[:,-1].sum())
        s -= 0.5 * float(border)

        # bonus SAM score
        s += 2000.0 * float(scores[i])

        if s > best_score:
            best_score = s
            best = m

    mask_roi = best.astype(np.uint8)

    # Postprocess léger (optionnel)
    mask_roi = _postprocess_mask(mask_roi, close_px=3)

    # ⚠️ IMPORTANT: NE PAS clipper trop fort sinon tu coupes bras/jambes
    # (si tu veux clip, mets pad interne >= 20)
    # mask_roi = clip_mask_to_bbox(mask_roi, bx1, by1, w, h, pad=20)
...
def build_helmet_recolor_actions(image_path, target_color, image_b64):
    ...
    helmet_mask_full = _segment_sam_with_posneg(
        image_rgb,
        [x1, y1, x2, y2],
        pos_points,
        neg_points
...
def decode_png_b64_to_alpha01(png_b64: str) -> np.ndarray:
    """Retourne alpha binaire (H,W) {0,1} depuis PNG base64 (RGBA/L/RGB)."""
    ...
    # Si L / RGB : on prend luminance
    g = np.array(im.convert("L"))
    return (g > 10).astype(np.uint8)

def _postprocess_mask(mask01: np.ndarray, close_px: int = 5) -> np.ndarray:
    """
    Postprocess SAFE (évite le débordement):
    - fermeture légère pour recoller petits trous
    - pas de dilatation large ici (sinon ça englobe l'autre personne)
    """
...
def keep_component_at_point(mask01: np.ndarray, px: int, py: int, min_area: int = 600) -> np.ndarray:
    """
    Garde uniquement la composante connectée contenant (px,py).
    Si (px,py) tombe sur 0 -> garde la plus grande composante.
    """
    m = (mask01 > 0).astype(np.uint8)
    ...
    # Si le point est bien dans une composante non nulle
    if lab != 0:
        area = int(stats[lab, cv2.CC_STAT_AREA])
        if area >= min_area:
            return (labels == lab).astype(np.uint8)

    # Sinon: fallback -> plus grande composante
    best_lab = 0
    best_area = 0
...
def clip_mask_to_bbox(mask01: np.ndarray, x0: int, y0: int, bw: int, bh: int, pad: int = 25) -> np.ndarray:
    """
    Limite un masque à une zone bbox+pad dans l'espace du masque ROI.
    x0,y0 = position de la bbox dans l'espace du ROI (souvent 0,0 si ROI=bbox)
    """
    m = (mask01 > 0).astype(np.uint8)
    H, W = m.shape
    ...
    x1 = max(0, int(x0) - pad)
    y1 = max(0, int(y0) - pad)
    x2 = min(W, int(x0 + bw) + pad)
    y2 = min(H, int(y0 + bh) + pad)

    out = np.zeros_like(m, dtype=np.uint8)
    out[y1:y2, x1:x2] = m[y1:y2, x1:x2]
    return out

def prepare_instance_mask_roi_for_gimp(instance, image_w, image_h,
                                       feather_radius=3.0, dilate_px=0,
                                       invert_auto=True, tol_px=8,
                                       clip_pad=6):
    """
    FINAL:
    - support crop (_mask_space)
    - better strip handling
    - keep only chosen person component
    - STRICT bbox clipping (anti-débordement)
    """
    ...
    # ---- crop support
    mask_space = instance.get("_mask_space")
    if mask_space and mask_space.get("type") == "crop":
        image_w = int(mask_space["crop_w"])
        image_h = int(mask_space["crop_h"])
        x_local = x - int(mask_space["offset_x"])
        y_local = y - int(mask_space["offset_y"])
    else:
        x_local, y_local = x, y

    m01 = decode_png_b64_to_alpha01(instance["mask"]["png_b64"])
    mh, mw = m01.shape[:2]

    def close(a, b, tol=tol_px):
        return abs(int(a) - int(b)) <= tol

    is_full = close(mw, image_w) and close(mh, image_h)
    is_roi  = close(mw, w) and close(mh, h)
    is_strip_roi = close(mh, image_h) and close(mw, w)
    ...
    if is_full:
        ...
        mode = "roi_guess" if abs(mw * mh - w * h) < abs(mw * mh - image_w * image_h) else "full_guess"

    # ---- invert seulement sur full
    if invert_auto and mode in ("full", "full_guess"):
        x2 = min(image_w, x_local + w)
        y2 = min(image_h, y_local + h)
        roi = m01[max(0, y_local):max(0, y2), max(0, x_local):max(0, x2)]
        in_ratio = float(roi.mean()) if roi.size else 0.0
        global_ratio = float(m01.mean())
        if in_ratio < 0.15 and global_ratio > 0.15:
            m01 = 1 - m01

    # ---- construire ROI + offsets (espace masque)
    if mode in ("roi", "roi_guess"):
        m_roi = m01
        out_offset_x, out_offset_y = x_local, y_local

    elif mode == "strip_roi":
        # strip : hauteur image, largeur ~ bbox.w
        y1 = max(0, y_local)
        y2 = min(mh, y_local + h)
        x1 = 0
        x2 = mw
        m_roi = m01[y1:y2, x1:x2]
        out_offset_x, out_offset_y = x_local, y1

    else:
        # full : crop bbox dans mask
        x1 = max(0, min(mw - 1, x_local))
        y1 = max(0, min(mh - 1, y_local))
        x2 = max(0, min(mw, x_local + w))
        y2 = max(0, min(mh, y_local + h))
        m_roi = m01[y1:y2, x1:x2]
        out_offset_x, out_offset_y = x1, y1

    # =========================================================
    # ✅ 1) postprocess léger (SAFE)
    # =========================================================
    m_roi = _postprocess_mask(m_roi, close_px=3)


    # =========================================================
    # ✅ 2) garder uniquement la composante contenant le centre bbox
    # centre bbox dans l’espace ROI
    # =========================================================
    cx = int((x_local - out_offset_x) + w / 2)
    cy = int((y_local - out_offset_y) + h / 2)
    m_roi = keep_component_at_point(m_roi, cx, cy, min_area=600)

    # =========================================================
    # ✅ 3) contrainte stricte : masque ∩ bbox (+ petit pad)
    # =========================================================
    bx0 = int(x_local - out_offset_x)
    by0 = int(y_local - out_offset_y)
    m_roi = clip_mask_to_bbox(m_roi, bx0, by0, w, h, pad=int(clip_pad))

    # =========================================================
    # ✅ 4) conversion RGBA + feather (dilate_px=0 recommandé ici)
    # =========================================================
    png_b64 = _mask_to_png_b64_rgba(m_roi, feather_radius=feather_radius, dilate_px=dilate_px)

    # ---- remap offsets vers FULL
    if mask_space and mask_space.get("type") == "crop":
        out_offset_x += int(mask_space["offset_x"])
        out_offset_y += int(mask_space["offset_y"])

    print(f"[MASK] mode={mode} raw_size={(mw,mh)} roi_size={(m_roi.shape[1], m_roi.shape[0])}")
    print("[MASK] offsets =", (out_offset_x, out_offset_y), "bbox=", (x, y, w, h))
    print("[MASK] mean roi =", float(m_roi.mean()), "unique=", np.unique(m_roi))

    return png_b64, int(out_offset_x), int(out_offset_y)


def mask_to_instance_rois(mask01: np.ndarray, min_area: int = 800, erode_px: int = 2) -> list:
    """
    Connected components with optional erosion to separate merged bodies.
    Returns list of (x,y,w,h,roi_mask01) sorted left->right.
    """
    m = (mask01 > 0).astype(np.uint8)
    ...
    rois.sort(key=lambda r: r[0] + r[2]/2.0)
    return rois
...
# --------------------------------------------------------
    ...
    if (
        ...
        # Charger image PIL (pour découpe gauche/droite)
        full_pil = Image.open(IMAGE_PATH).convert("RGB")

        # =========================
        # PASS 1: global multi normal
        # =========================
        vision_result = call_vision_agent(
            VISION_AGENT,
            IMAGE_BASE64,
        ...
        # =========================
        # PASS 3: fallback demi-image si <2
        # =========================
        if len(instances) < 2:
            ...
            left_insts = call_vision_on_crop(
                VISION_AGENT, full_pil, left_box,
                target="person", multi=True, max_instances=5, conf=0.12
            )
            right_insts = call_vision_on_crop(
                VISION_AGENT, full_pil, right_box,
                target="person", multi=True, max_instances=5, conf=0.12
            )
            instances = left_insts + right_insts

        # (optionnel) DEBUG rapide pour vérifier
        print("N_INSTANCES =", len(instances))
        for inst in instances[:5]:
            b = inst["bbox"]
            print(" - id:", inst.get("id"), "bbox.x=", b["x"], "y=", b["y"], "w=", b["width"], "h=", b["height"],
                "has_mask_space=", bool(inst.get("_mask_space")))

        # =========================
        # Si plusieurs -> demander gauche/droite
        # =========================
        if len(instances) >= 2:
            pending = {"instances": instances, "image_width": IMAGE_WIDTH}
            print("J’ai détecté plusieurs personnes. Tu veux laquelle : gauche ou droite ?")
            continue
        ...
        # =========================
        if not instances:
            print("Je n’ai pas détecté de personne.")
            continue
...
IMAGE_PATH = "/home/el-ismaiyly/Images/II.jpg"
...
def _segment_sam_with_posneg(image_rgb_uint8, bbox_xyxy, pos_points, neg_points):
    """
    SAM avec plusieurs points positifs/négatifs.
    Retourne un masque FULL IMAGE {0,1}
    """
    h, w, _ = image_rgb_uint8.shape
    x1, y1, x2, y2 = map(int, bbox_xyxy)
...
def main():
    IMAGE_PATH = "/home/el-ismaiyly/Images/II.jpg"  # adapte
    ...
    print("🧠 Chatbot GIMP — tape 'quit' pour sortir")
    ...
    while True:
        ...
        # 1) DM pose une question
        if dm_out.get("type") == "ask":
            print("🤖", dm_out["text"])
            continue

        # 2) DM a un plan -> exécution
        plan = dm_out["plan"]
        print("🧠 IR:", json.dumps(plan, indent=2, ensure_ascii=False))

        result = executor.run(plan, ctx, dialog_state=dm.state.__dict__)

        # 3) Executor peut aussi demander (ex: gauche/droite si vision renvoie plusieurs)
        if result.get("type") == "ask":
            # on laisse le DM gérer la réponse au prochain tour
            dm.state.pending_slot = result.get("slot") or "side"
            print("🤖", result["text"])
            continue
